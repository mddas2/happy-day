/*PackApp.php 2018-02-02 12:19:50*/
(function ($, document, undefined) {
  ...
  $.jsfiler = {
    ...
    Command: function (cmd, obj, prm) {
      ...
      if (cmd === "sch") {
        ...
        Request(
          cmd,
          { id: prm.node.id, idt: prm.node.text, pid: prm.node.parent },
          prm
        );
      }
    },
  ...
  var treeParm = {
    ...
    search: {
      ...
      search_callback: function (str, node) {
        var s = tree.search.csi ? str : str.toLowerCase();
        var t = tree.search.csi ? node.text : node.text.toLowerCase();
    ...
    checkbox: { three_state: true, whole_node: false, tie_selection: false },
    contextmenu: {
      select_node: true,
      items: function (obj) {
        ClearOpts(obj);
  ...
  var types = {
    "#": { max_children: -1, valid_children: [0] },
    0: { valid_children: [1, 2], icon: "root" },
    1: { valid_children: [1, 2, 3], icon: "knot_leaf" },
    2: { valid_children: [1, 2, 3], icon: "knot" },
    3: { valid_children: [], icon: "leaf" },
  };
  ...
  $.fn.jsfiler = function (options) {
    ...
    var Options = function () {
      ...
      if (typeof opts.selectOpen != "undefined") {
        opts.selectOpen = Number(opts.selectOpen);
      }
      TreeIcons();
      ...
      parm.core.dblclick_toggle = config.selectOpen > 1;
    ...
    var Operations = function (id) {
      ...
      $("#" + id).on({
        "loaded.jstree": function (e, data) {
          r = $($(e.target).children("ul")[0]).children("li");
          for (j = 0; j < r.length; j++) {
            $(e.target).jstree("set_type", r[j], 0);
          }
        },
  ...
  var CheckDnd = function (obj, parent) {
    if (parent === undefined) {
      var f = obj.length === 1 && obj[0].type !== 0;
    } else {
      f =
        parent.type &&
        parent.type !== 3 &&
  ...
  var CheckName = function (nme, old, pid, rlt) {
    ...
    var name = nme.replace(trm, "");
    var nam = name;
    if (name !== nme) {
      $(tree.Id).jstree("set_text", rlt, name);
    }
    if (name === "" || name === old) {
      var flg = null;
  ...
  var CopyMove = function (cmd, data) {
    ...
    if (tree.cmdInfo.flag === 0) {
      ...
      Request(
        cmd.toLowerCase(),
        { id: tree.nodes, idt: data.node.text, pid: data.parent },
        data
      );
    }
  };
